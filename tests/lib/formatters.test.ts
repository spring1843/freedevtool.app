...
describe('HTML Formatting', () => {
  ...
  it('should validate and report unclosed tags', () => {
    ...
    expect(result.warnings!.some(issue => 
      issue.message.includes('Unclosed tag <p>')
    )).toBe(true)
  })
  ...
  it('should handle nested tags correctly', () => {
    const input = '<div><article><header><h1>Title</h1></header><main><p>Content</p></main></article></div>'
    const result = formatHTML(input)
  ...
  it('should preserve attributes', () => {
    const input = '<div class="container" id="main"><p style="color: red;">Text</p></div>'
    const result = formatHTML(input)
...
.other {
  margin: 10px;
    ...
    expect(result.error).toBeUndefined()
    expect(result.formatted).toBe('.class{color:red;background:blue}.other{margin:10px}')
  })

  it('should handle CSS comments', () => {
    const input = '/* Main styles */ .class { color: red; /* Important */ background: blue; }'
    const result = formatCSS(input, false)
  ...
  it('should handle media queries', () => {
    const input = '@media screen and (max-width: 768px) { .class { display: none; } }'
    const result = formatCSS(input, false)
