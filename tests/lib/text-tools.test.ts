...
describe('Text Tools', () => {
  ...
  try {
    ...
    const textToolsPath = resolve(__dirname, '../../client/src/lib/text-tools.ts')
    const textToolsContent = readFileSync(textToolsPath, 'utf-8')
    
    // Extract exported function names from the file content
    const exportMatches = textToolsContent.match(/export\s+(function\s+\w+|const\s+\w+|let\s+\w+|var\s+\w+)/g) || []
    ...
    it('should indicate text-tools module status', () => {
      console.log('Text tools module not found or not accessible for testing')
