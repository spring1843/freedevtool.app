 ...
 */
function validateParam(value: any, rule: ValidationRule, defaultValue: any): any {
  try {
    switch (rule.type) {
      case 'string':
        ...
        if (rule.allowedValues && !rule.allowedValues.includes(sanitized)) return defaultValue;
        return sanitized;
      ...
      case 'array':
        ...
        if (rule.arrayItemPattern) {
          const validItems = items.filter(item => rule.arrayItemPattern!.test(item));
          if (validItems.length !== items.length) return defaultValue;
        }
        
        if (rule.allowedValues) {
          const validItems = items.filter(item => rule.allowedValues!.includes(item));
          if (validItems.length !== items.length) return defaultValue;
          return validItems;
        }
 ...
 */
export function getParam(key: string, defaultValue?: string): string | undefined;
 ...
 */
export async function copyShareableURL(params: URLShareParams): Promise<boolean> {
  try {
...
export const commonValidationSchemas = {
  ...
  alphanumeric: { type: 'string' as const, pattern: /^[a-zA-Z0-9]*$/, maxLength: 100 },
  safePath: { type: 'string' as const, pattern: /^[a-zA-Z0-9/_-]*$/, maxLength: 200 },
  dateString: { type: 'string' as const, pattern: /^\d{4}-\d{2}-\d{2}$/, maxLength: 10 },
  timeString: { type: 'string' as const, pattern: /^\d{2}:\d{2}(:\d{2})?$/, maxLength: 8 },
  
  // Common enums
  frequency: { type: 'enum' as const, allowedValues: ['daily', 'weekly', 'monthly', 'yearly'] },
