...
export function calculateDebtPayment(
  ...
  if (monthlyRate > 0) {
    ...
    if (monthlyPayment <= minimumInterestPayment) {
      throw new Error(
        `Monthly payment ($${monthlyPayment.toFixed(2)}) must be higher than the monthly interest ($${minimumInterestPayment.toFixed(2)}) to pay off the debt`
      );
    }
  }
...
export function calculateMultipleDebts(
  ...
  if (totalPayment < totalMinimum) {
    throw new Error(
      "Total payment must be at least the sum of minimum payments"
    );
  }
  ...
  const totalInterestPaid = monthlySummary.reduce(
    (sum, month) => sum + month.totalInterest,
    0
  );
  const totalAmountPaid = monthlySummary.reduce(
