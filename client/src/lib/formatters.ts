...
export async function formatJavaScript(
  ...
  try {
    ...
    // Custom JavaScript beautifier since Prettier's babel parser isn't available
    ...
    return {
      formatted: input,
      error: `JavaScript formatting error: ${error instanceof Error ? error.message : "Unknown error"}`,
    };
  }
}
