import type { DiffLine, RegexMatch, SortOrder, SortType, TextStats } from "@/types/tools";

// Enhanced diff with word-level highlighting for modified lines
function getWordDiff(line1: string, line2: string): { 
  type: 'modified';
  content: string; 
...
export function computeTextDiff(text1: string, text2: string): {
  diff: DiffLine[];
  stats: {
    linesAdded: number;
    linesRemoved: number;
  ...
  while (i < lines1.length || j < lines2.length) {
    if (i >= lines1.length) {
      ...
      if (similarity > 0.3) { // If lines are somewhat similar, show word-level diff
...
export function testRegex(pattern: string, text: string, flags = 'g'): {
  matches: RegexMatch[];
  error?: string;
} {
  try {
