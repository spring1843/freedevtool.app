...
import { CalendarDays, Clock, Calculator, Copy, Check, RotateCcw } from "lucide-react";
...
export default function DateTimeDiff() {
  ...
  const { fields, updateField, resetFields } = usePersistentForm('datetime-diff', getDefaultValues());
  ...
  const presets = [
    ...
    // Historical dates
    { label: "From year 1 AD to now", type: "historical", startDate: "0001-01-01", startTime: "00:00" },
    { label: "From year 1000 to now", type: "historical", startDate: "1000-01-01", startTime: "00:00" },
    { label: "From Unix epoch to now", type: "historical", startDate: "1970-01-01", startTime: "00:00" },
    { label: "From Y2K to now", type: "historical", startDate: "2000-01-01", startTime: "00:00" },
    { label: "From 2010 to now", type: "historical", startDate: "2010-01-01", startTime: "00:00" },
    
    // Far future
    { label: "100 years from now", days: 36525 },
    { label: "500 years from now", days: 182625 },
    { label: "1000 years from now", days: 365250 },
    { label: "To year 3000", type: "future", endDate: "3000-01-01", endTime: "00:00" },
    { label: "To year 10000", type: "future", endDate: "10000-01-01", endTime: "00:00" }
  ];
  ...
  const applyPreset = (preset: typeof presets[0]) => {
    ...
    if (preset.type === 'historical') {
      ...
      if (preset.minutes) future.setMinutes(future.getMinutes() + preset.minutes);
  ...
  const formatNumber = (num: number): string => Math.floor(num).toLocaleString();
  ...
  const getReadableDuration = (diff: DateTimeDifference): string => {
    ...
    if (diff.weeks > 0) parts.push(`${diff.weeks} week${diff.weeks !== 1 ? 's' : ''}`);
    ...
    if (diff.seconds > 0 && parts.length < 3) parts.push(`${diff.seconds} second${diff.seconds !== 1 ? 's' : ''}`);
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Calculate the precise difference between two dates and times with detailed breakdowns. Explore fascinating time spans from historical dates to far future dates.
        </p>
      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {/* Quick Intervals */}
                <div>
                  <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Quick Future Intervals</h4>
                  <div className="grid grid-cols-3 gap-2">
                    {presets.filter(p => !p.type).map((preset, index) => (
                      <Button
                        key={index}
                        variant="outline"
                        size="sm"
                ...
                {/* Historical */}
                <div>
                  <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Historical Periods</h4>
                  <div className="grid grid-cols-2 gap-2">
                    {presets.filter(p => p.type === 'historical').map((preset, index) => (
                      <Button
                        key={index}
                        variant="outline"
                        size="sm"
                ...
                {/* Far Future */}
                <div>
                  <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Far Future</h4>
                  <div className="grid grid-cols-2 gap-2">
                    {presets.filter(p => p.type === 'future' || (p.days && p.days > 10000)).map((preset, index) => (
                      <Button
                        key={index}
                        variant="outline"
                        size="sm"
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-4">
                Click any preset to automatically set dates and calculate fascinating time differences
              </p>
            </CardContent>
          </Card>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                      ...
                      <div className="text-sm text-blue-700 dark:text-blue-300">
                        Total: {formatNumber(difference.totalDays)} days, {difference.totalHours % 24} hours, {difference.totalMinutes % 60} minutes
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span className="text-slate-600 dark:text-slate-400">Seconds:</span>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {[
                      { label: 'Total Years', value: (difference.totalYears).toFixed(2), key: 'totalYears' },
                      { label: 'Total Months', value: formatNumber(difference.totalMonths), key: 'totalMonths' },
                      { label: 'Total Weeks', value: formatNumber(difference.totalWeeks), key: 'totalWeeks' },
                      { label: 'Total Days', value: formatNumber(difference.totalDays), key: 'totalDays' },
                      { label: 'Total Hours', value: formatNumber(difference.totalHours), key: 'totalHours' },
                      { label: 'Total Minutes', value: formatNumber(difference.totalMinutes), key: 'totalMinutes' },
                      { label: 'Total Seconds', value: formatNumber(difference.totalSeconds), key: 'totalSeconds' },
                    ].map(({ label, value, key }) => (
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-center py-12 text-slate-500 dark:text-slate-400">
                  ...
                  <p className="text-sm">Enter both start and end dates to see the difference</p>
                </div>
              </CardContent>
            </Card>
          )}
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <h4 className="font-semibold mb-3 text-purple-700 dark:text-purple-300">Project Planning</h4>
            ...
            <div>
              <h4 className="font-semibold mb-3 text-green-700 dark:text-green-300">Personal Milestones</h4>
              <ul className="text-sm space-y-1 text-slate-600 dark:text-slate-400">
            ...
            <div>
              <h4 className="font-semibold mb-3 text-blue-700 dark:text-blue-300">Business Applications</h4>
              <ul className="text-sm space-y-1 text-slate-600 dark:text-slate-400">
