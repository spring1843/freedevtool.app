...
import { defaultWorldClockCities, allTimezones, continentalCities, getTimeForTimezone, getDateForTimezone, getTimezoneOffset, getUserTimezone } from "@/lib/time-tools";
...
export default function WorldClock() {
  const [currentTimes, setCurrentTimes] = useState<Record<string, { time: string; date: string; offset: string }>>({});
  const [continentalTimes, setContinentalTimes] = useState<Record<string, { time: string; date: string; offset: string }>>({});
  const [displayedCities, setDisplayedCities] = useState(defaultWorldClockCities);
  ...
  useEffect(() => {
    const updateTimes = () => {
      ...
      const contTimes: Record<string, { time: string; date: string; offset: string }> = {};
      Object.values(continentalCities).flat().forEach(city => {
  ...
  const getLocalDate = () => new Date().toLocaleDateString('en-US', {
      weekday: 'short',
      year: 'numeric',
      month: 'short',
      day: 'numeric',
  ...
  const filteredTimezones = allTimezones.filter(city =>
  ...
  const removeClock = (timezoneToRemove: string) => {
    const timezone = displayedCities.find(city => city.timezone === timezoneToRemove);
    if (!timezone) return;
    
    setDisplayedCities(displayedCities.filter(city => city.timezone !== timezoneToRemove));
        ...
        {showAddClock ? <CardContent className="space-y-4">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <Button onClick={addClock} className="w-full" data-testid="add-clock-confirm">
                  <Plus className="w-4 h-4 mr-2" />
                  Add Clock
                </Button>
              </div>
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400">
              <p><strong>Currently showing:</strong> {displayedCities.length} clocks</p>
              <p><strong>Available timezones:</strong> {filteredTimezones.length} found</p>
            </div>
          </CardContent> : null}
      </Card>
        ...
        <div className="space-y-8">
          {Object.entries(continentalCities)
            .sort(([, citiesA], [, citiesB]) => {
              ...
              const minOffsetA = Math.min(...citiesA.map(city => {
                const timeData = continentalTimes[city.timezone];
                return timeData ? parseInt(timeData.offset.replace(/[^\d-]/g, ''), 10) : 0;
              }));
              const minOffsetB = Math.min(...citiesB.map(city => {
                const timeData = continentalTimes[city.timezone];
                return timeData ? parseInt(timeData.offset.replace(/[^\d-]/g, ''), 10) : 0;
              }));
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                  {cities
                    .sort((a, b) => {
                      ...
                      const offsetA = parseInt(timeDataA.offset.replace(/[^\d-]/g, ''), 10);
                      const offsetB = parseInt(timeDataB.offset.replace(/[^\d-]/g, ''), 10);
                      return offsetA - offsetB;
                    })
                    .map((city, index) => {
                    const timeData = continentalTimes[city.timezone];
                    const isInCustomClocks = displayedCities.some(existing => existing.timezone === city.timezone);
                    
                    return (
                      <div key={city.timezone} className="text-center p-4 border border-slate-200 dark:border-slate-700 rounded-lg transition-all hover:shadow-sm relative">
                        {/* Remove button for cities that are in custom clocks */}
                        {isInCustomClocks ? <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {displayedCities
            .sort((a, b) => {
              ...
              if (!timeDataA || !timeDataB) return 0;
              const offsetA = parseInt(timeDataA.offset.replace(/[^\d-]/g, ''), 10) || 0;
              const offsetB = parseInt(timeDataB.offset.replace(/[^\d-]/g, ''), 10) || 0;
              return offsetA - offsetB;
            })
            .map((city, index) => {
            const timeData = currentTimes[city.timezone];
            const _isCustom = !defaultWorldClockCities.some(defaultCity => defaultCity.timezone === city.timezone);
            
            return (
              <Card key={city.timezone} className="transition-all hover:shadow-md">
                ...
                <CardContent className="pt-0">
                  {timeData ? <div className="text-center">
