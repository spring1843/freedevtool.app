  ...
  const convertDate = () => {
    ...
    if (!date) {
      setFormats([{
        ...
        description: "Supported: Unix timestamps (seconds/milliseconds), ISO 8601, or any standard date format",
        category: "Error"
      }]);
      return;
    }
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">
              Convert between 20 essential date formats: Unix timestamps, ISO standards, RFC formats, regional formats, and database formats
            </p>
          </div>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Supports: Unix timestamps (seconds/milliseconds), ISO 8601, RFC formats, human-readable dates
            </p>
          </div>
      ...
      {formats.length > 0 && (
        <Card>
          ...
          <CardContent>
            ...
            {['Timestamp', 'ISO Standards', 'RFC Standards', 'Regional', 'Database', 'Human Readable', 'Web/API'].map(category => {
              const categoryFormats = formats.filter(f => f.category === category);
              if (categoryFormats.length === 0) return null;
              
              return (
                <div key={category} className="mb-6">
                  <h3 className="text-lg font-medium mb-3 text-blue-600 dark:text-blue-400">{category}</h3>
            ...
            {formats.filter(f => f.category === 'Error').map((format, index) => (
              <div key={index} className="p-4 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/20">
                <div className="text-red-600 dark:text-red-400 font-medium">{format.name}</div>
                <div className="text-red-700 dark:text-red-300 text-sm mt-1">{format.value}</div>
                <div className="text-xs text-red-600 dark:text-red-400 mt-2">{format.description}</div>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div>
              <h4 className="font-semibold mb-3 text-blue-600 dark:text-blue-400">Timestamp Formats</h4>
              <ul className="space-y-2 text-xs">
                <li><code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">1699123456</code> - Unix timestamp (seconds)</li>
                <li><code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">1699123456000</code> - Unix milliseconds</li>
                <li><code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">2024-01-15T14:30:45Z</code> - ISO 8601</li>
                <li><code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">Jan 15, 2024</code> - Human readable</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold mb-3 text-green-600 dark:text-green-400">Output Categories</h4>
              <ul className="space-y-2 text-xs">
                <li><strong>Timestamps:</strong> Unix seconds & milliseconds</li>
                <li><strong>ISO Standards:</strong> ISO 8601 full, date-only, time-only</li>
                <li><strong>RFC Standards:</strong> RFC 2822, RFC 3339</li>
                <li><strong>Regional:</strong> US, European, ISO numeric formats</li>
                <li><strong>Database:</strong> SQL datetime, MongoDB ObjectId</li>
                <li><strong>Human Readable:</strong> Full text, short text, 12/24h time</li>
                <li><strong>Web/API:</strong> HTTP headers, JSON, cookies</li>
              </ul>
            </div>
          </div>
          <div className="text-xs text-gray-600 dark:text-gray-400 mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded">
            <strong>Auto-detection:</strong> Simply paste any timestamp, ISO date, or human-readable date. 
            The converter automatically detects the format and converts to all {DATE_FORMATS.length} essential formats.
          </div>
        </CardContent>
      </Card>
