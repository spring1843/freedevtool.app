...
export function Layout({ children }: LayoutProps) {
  ...
  const { isDemoRunning, isDemoPaused, currentDemoTool, demoProgress, demoSpeed, stopDemo, skipToNext, setDemoSpeed, pauseDemo, resumeDemo } = useDemo();
      ...
      if (event.ctrlKey || event.metaKey) {
        ...
        Object.values(toolsData).forEach(section => {
          section.tools.forEach(tool => {
            ...
            if (shortcutParts.includes('Ctrl') && !event.ctrlKey && !event.metaKey) matches = false;
            if (shortcutParts.includes('Shift') && !event.shiftKey) matches = false;
  ...
  return (
    <TooltipProvider>
      ...
      {/* Collapsible Header */}
      <div className={`transition-all duration-300 ${headerCollapsed ? 'h-0' : 'h-auto'} ${headerCollapsed ? 'overflow-hidden' : 'overflow-visible'} relative z-50`}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-1">
                ...
                <Select value={demoSpeed} onValueChange={setDemoSpeed}>
                  ...
                  <SelectContent>
                    <SelectItem value="slow" className="text-xs">Slow (8s)</SelectItem>
                    <SelectItem value="normal" className="text-xs">Normal (5s)</SelectItem>
                    <SelectItem value="fast" className="text-xs">Fast (3s)</SelectItem>
                    <SelectItem value="very-fast" className="text-xs">Very Fast (1.5s)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{isDemoPaused ? 'Resume Demo Tour' : 'Pause Demo Tour'}</p>
                </TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
      ...
      <div className={`flex justify-end pr-4 transition-all duration-300 ${headerCollapsed ? 'py-1' : 'py-0'}`}>
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <ChevronUp className={`w-3 h-3 transition-transform duration-200 ${headerCollapsed ? 'rotate-180' : ''}`} />
            </Button>
          </TooltipTrigger>
          <TooltipContent>
      ...
      <div className="flex flex-1">
        ...
        <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen} modal={false}>
          <SheetContent 
            ...
            style={{ marginTop: '4rem', height: 'fit-content', maxHeight: 'calc(100vh - 4rem)' }}
          >
            <SheetHeader className="sr-only">
              <SheetTitle>Navigation Menu</SheetTitle>
              <SheetDescription>
                Navigation menu with all available developer tools organized by category
              </SheetDescription>
            </SheetHeader>
      ...
      <footer className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-6 py-3">
        <div className="flex items-center justify-between text-sm text-slate-600 dark:text-slate-400">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <kbd className="px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded text-xs">Ctrl+M</kbd>
            <span>Menu</span>
            <span className="w-1 h-1 bg-slate-300 rounded-full" />
            <kbd className="px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded text-xs">Ctrl+K</kbd>
            <span>Search</span>
            <span className="w-1 h-1 bg-slate-300 rounded-full" />
