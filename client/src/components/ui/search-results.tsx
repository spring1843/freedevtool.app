...
export function SearchResults({ results, onResultClick, className = "", selectedIndex = -1 }: SearchResultsProps) {
      ...
      {results.map((result, index) => {
        ...
        return (
          <Link key={`${result.path}-${index}`} href={result.path}>
            ...
            <ExternalLink className={`w-4 h-4 ml-2 transition-opacity ${
              isSelected 
