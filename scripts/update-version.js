...
async function updateVersion(newVersion) {
  try {
    ...
    try {
      ...
      execSync(`git commit -m "chore: bump version to ${cleanVersion}"`, {
        stdio: "inherit",
      });
      ...
      console.warn(
        "⚠️ Git operations failed (this might be expected in CI):",
        gitError.message
      );
    }
  } catch (error) {
...
if (!versionPattern.test(newVersion)) {
  console.error(
    "❌ Invalid version format. Please use semantic versioning (e.g., 1.2.0 or v1.2.0)"
  );
  process.exit(1);
}
